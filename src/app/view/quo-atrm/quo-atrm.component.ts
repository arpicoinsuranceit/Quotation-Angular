...
@Component({
  selector: 'app-quo-atrm',
  templateUrl: './quo-atrm.component.html',
  styleUrls: ['./quo-atrm.component.css']
})
export class QuoAtrmComponent implements OnInit {

  @ViewChild(AtrmPersonalInfoComponent) atrmPersonolInfoComponent: AtrmPersonalInfoComponent;
  @ViewChild(AtrmAdditionalBenefComponent) atrmBenefictInfoComponent: AtrmAdditionalBenefComponent;
  ...
  constructor(private loginService: LoginService, private saveAtrmQuotationService: SaveAtrmQuotationService, private router: Router, private route: ActivatedRoute,
    ...
    this.route.params.subscribe(params => {
      this.qdId = params.id;
    },error => {swal("Error", "Error code - 410 <br> ","error")});
  ...
  sendQuo() {
    ...
    if (this._quotationCalculation._personalInfo != null) {
      if (this.personalInfo._plan._term > 45 || this.personalInfo._plan._term < 5) {
      ...
      this.saveAtrmQuotationService.getQouATRMCal(this._quotationCalculation).subscribe(response => {
        ...
        this.summeryInfo._summery.at6 = response.json().guaranteed.toLocaleString();
        ...
        swal("Error", "Error code - 411 <br> ","error");
        document.onkeydown = function (e) { return true; }
        this.isDisableDiv = false;
      });
    }
          ...
          if (this.personalInfo._mainlife._mOccupation != "285") {
            if (this.activeSp == "1") {
              if (this.personalInfo._spouse._sOccupation != "285") {
                ...
                this.isDisableDiv = true;
                document.onkeydown = function (e) { return false; }
                this.saveAtrmQuotationService.saveAtrm(this._invpSaveQuotation).subscribe(response => {
                  this.isDisableDiv = false;
                  document.onkeydown = function (e) { return true; }
                  ...
                  swal("Error", "Error code - 412 <br> ","error");
                  document.onkeydown = function (e) { return true; }
                  this.isDisableDiv = false;
                });
              } else {
        ...
        if (this.activeSp == "1") {
          if (this.personalInfo._spouse._sOccupation != "285") {
            ...
            this.saveAtrmQuotationService.editAtrm(this._invpSaveQuotation, this.qdId).subscribe(response => {
              ...
              swal("Error", "Error code - 413 <br> ","error");
              document.onkeydown = function (e) { return true; }
              this.isDisableDiv = false;
            });
          } else {
  ...
  editCal() {
    this.saveAtrmQuotationService.getAtrmQuotationDetailsForEdit(this.qdId).subscribe(response => {
      ...
      swal("Error", "Error code - 414 <br>","error");
      document.onkeydown = function (e) { return true; }
      this.isDisableDiv = false;
    });
  }
  ...
  clearquo() {
    this.atrmPersonolInfoComponent.loadDefault();
    this.atrmBenefictInfoComponent.loadDefault();
    this.sendQuo();
  }

 clearbenef (e) {

    this.atrmBenefictInfoComponent.loadDefaultNew();
    //this.sendQuo();
  }

  clearbenefAll(e) {

    this.atrmBenefictInfoComponent.loadDefaultBSAChange();
    //this.sendQuo();
  }
  ...
  calPreviousRiskM(e) {
    ...
    this.dashboardService.getSumAtRiskMainLife(e).subscribe(resp => {
      this.isDisableDiv = false;
      this.atrmPersonolInfoComponent.loadDOBFromNic();
      document.onkeydown = function (e) { return true; }
      if (resp.json()) {
      ...
      swal("Error", "Error code - 415 <br> ","error");
      document.onkeydown = function (e) { return true; }
      this.isDisableDiv = false;
    });
  }
  ...
  calPreviousRiskS(e) {
    ...
    this.dashboardService.getSumAtRiskMainLife(e).subscribe(resp => {
      this.isDisableDiv = false;
      this.atrmPersonolInfoComponent.loadSpouseDOBFromNic();
      document.onkeydown = function (e) { return true; }
      if (resp.json()) {
      ...
      swal("Error", "Error code - 416 <br> ","error");
      document.onkeydown = function (e) { return true; }
      this.isDisableDiv = false;
    });
  }
