import { PlanArp, Plan } from './../../../model/plan';
...
@Component({
  selector: 'app-arp-personal-info',
  templateUrl: './arp-personal-info.component.html',
  styleUrls: ['./arp-personal-info.component.css']
})
export class ArpPersonalInfoComponent implements OnInit {
  ...
  mainLifeForm = new FormGroup({
    ...
    mName: new FormControl('', [Validators.required,Validators.maxLength(200)]),
  ...
  spouseForm = new FormGroup({
    ...
    sName: new FormControl('', [Validators.required,Validators.maxLength(200)]),
  ...
  change(e) {
    ...
    if (this._personalInfo._plan._frequance == "Single Premium") {
      this._personalInfo._plan._payingterm = "S";
    }else{
      if (this._personalInfo._plan._payingterm == "S") {
        this._personalInfo._plan._payingterm = "5";
      }
    }

    if (this._personalInfo._plan._payingterm != "S") {
      if ((this._personalInfo._plan._term - parseInt(this._personalInfo._plan._payingterm)) < 5) {
        swal("Error", "Policy Term must be grater than Paying Term", "error");
        return;
      }
    }

    if (this._personalInfo._plan._term > 30 || this._personalInfo._plan._term < 5) {
  ...
  checkValidity() {
    ...
    if (this.activeSp == "1") {
      if ((this._personalInfo._mainlife._mName != undefined && this._personalInfo._mainlife._mName != "") &&
        ...
        if (this._personalInfo._mainlife._mAge == undefined || this._personalInfo._mainlife._mAge == 0) {
        ...
        if (this._personalInfo._spouse._sAge == undefined || this._personalInfo._spouse._sAge == 0) {
  ...
  /*setDOBReadonly(e: NgbTabChangeEvent){
    //alert("called");

    if (e.nextId === 'tab-mainLife') {
      if(this._personalInfo._mainlife._mNic != "" && this._personalInfo._mainlife._mNic != undefined){
        this.mainLifeForm.get("mDob").disable();
      }else{
        this.mainLifeForm.get("mDob").enable();
      }
    ...
    if (e.nextId === 'tab-spouse') {
      if(this._personalInfo._spouse._sNic != "" && this._personalInfo._spouse._sNic != undefined){
        this.spouseForm.get("sDob").disable();
      }else{
        this.spouseForm.get("sDob").enable();
      }
