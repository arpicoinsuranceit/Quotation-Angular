...
@Component({
  ...
  templateUrl: './quo-asfp.component.html',
  styleUrls: ['./quo-asfp.component.css']
})
export class QuoAsfpComponent implements OnInit {
  ...
  isImgSFPOGActive = true;
  isImgSFPOActive = false;
  ...
  @ViewChild(AsfpPersonalInfoComponent) asfpPersonalInfoComponent: AsfpPersonalInfoComponent;
  ...
  constructor(private saveAsfpQuotationService: SaveAsfpQuotationService, private router: Router, private route: ActivatedRoute, private loginService: LoginService,
    ...
    this.route.params.subscribe(params => {
      this.qdId = params.id;
    }, error => { swal("Error", "Error code - 310 <br>", "error") });

    this.personalInfo._plan._msfb = 10000;
    this.personalInfo._plan._frequance = "Monthly";
    this.personalInfo._plan._term = 10;
    this.personalInfo._plan._bsa = 600000;
    this.personalInfo._plan._nomineeAge = 0;
    ...
    this.personalInfo._plan._nomineeName = "";
  }
  ...
  sendQuo() {
    ...
    if (this._plan._term < 5 || this._plan._term > 40 || Number.isNaN(this._plan._term)) {
      swal("Error", "Term must be grater than 5 and less than 40", "error");
      return;
    }
      ...
      if (parseInt(this._quotationCalculation._personalInfo.mage.toString()) + parseInt(this._plan._term.toString()) <= 70) {
        if (parseInt(this._plan._nomineeAge.toString()) + parseInt(this._plan._term.toString()) <= 24) {
          ...
          this.saveAsfpQuotationService.getQouCal(this._quotationCalculation).subscribe(response => {
            ...
            this.personalInfo._plan._bsa = response.json().l2;
            ...
            this.summeryInfo._protection.SFPO = response.json().sfpo;
            this.summeryInfo._protection.SFPOTerm = response.json().sfpoTerm;
            ...
            swal("Error", "Error code - 311 <br> ", "error");
            document.onkeydown = function (e) { return true; }
            this.isDisableDiv = false;
          });
        } else {
          swal("Error", "Term must be " + (24 - this._plan._nomineeAge) + "", "error");
        }
      } else {
        swal("Error", "Term must be " + (70 - this._quotationCalculation._personalInfo.mage) + "", "error");
      }
                ...
                if (this.personalInfo._spouse._sOccupation != "285") {
                  if (this.personalInfo._spouse._sAge < 61 && this.personalInfo._spouse._sAge > 17) {
                    ...
                    this.saveAsfpQuotationService.saveInvp(this._invpSaveQuotation).subscribe(response => {
                      ...
                      swal("Error", "Error code - 312 <br> ", "error");
                      document.onkeydown = function (e) { return true; }
                      this.isDisableDiv = false;
                    });
                  } else {
              ...
              if (this.activeSp == "1") {
                if (this.personalInfo._spouse._sOccupation != "285") {
                  if (this.personalInfo._spouse._sAge < 61 && this.personalInfo._spouse._sAge > 17) {
                    ...
                    this.isDisableDiv = true;
                    document.onkeydown = function (e) { return false; }
                    this.saveAsfpQuotationService.editAsfp(this._invpSaveQuotation, this.qdId).subscribe(response => {
                      this.isDisableDiv = false;
                      document.onkeydown = function (e) { return true; }
                      if (response.json().status == "Success") {
                      ...
                      swal("Error", "Error code - 313 <br>", "error");
                      document.onkeydown = function (e) { return true; }
                      this.isDisableDiv = false;
                    });
                  } else {
  ...
  editCal() {
    this.saveAsfpQuotationService.getAsfpQuotationDetailsForEdit(this.qdId).subscribe(response => {
      ...
      this._quotationCalculation._personalInfo.msfb = this._quotationCalculation._personalInfo.bsa / (this._quotationCalculation._personalInfo.term * 12);
      this.personalInfo._plan._msfb = this._quotationCalculation._personalInfo.msfb;
      ...
      for (var q in this._mainLifeBenefits) {
        ...
        switch (this._mainLifeBenefits[q].benfName) {
          case "SFPO": {
            ...
            this.isImgSFPOActive = true;
            this._mainLifeBenefForm.get("SFPO").get("isActice").setValue(true);
            this._mainLifeBenefForm.get("SFPO").get("sumAssured").setValue(this._mainLifeBenefits[q].riderSum);
            this._mainLifeBenefForm.get("SFPO").get("sumAssured").enable();
      ...
      swal("Error", "Error code - 314 <br>", "error");
      document.onkeydown = function (e) { return true; }
      this.isDisableDiv = false;
    });
  }
  ...
  clearquo() {
    this.asfpPersonalInfoComponent.loadDefault();
    this.asfpAdditionalBenefComponent.loadDefault();
    this.sendQuo();
  ...
  clearbenef(e) {
    ...
    this.asfpAdditionalBenefComponent.loadDefaultNew();
    //this.sendQuo();
  }

  clearbenefAll(e) {

    this.asfpAdditionalBenefComponent.loadDefaultBSAChange();
    //this.sendQuo();
  }
  ...
  calPreviousRiskM(e) {
    if (e.length > 0) {
      ...
      this.dashboardService.getSumAtRiskMainLife(e).subscribe(resp => {
        this.isDisableDiv = false;
        document.onkeydown = function (e) { return true; }
        this.asfpPersonalInfoComponent.loadDOBFromNic();
        ...
        swal("Error", "Error code - 315 <br> ", "error");
        document.onkeydown = function (e) { return true; }
        this.isDisableDiv = false;
      });
    }
  ...
  calPreviousRiskS(e) {
    if (e.length > 0) {
      ...
      this.dashboardService.getSumAtRiskMainLife(e).subscribe(resp => {
        this.isDisableDiv = false;
        this.asfpPersonalInfoComponent.loadSpouseDOBFromNic();
        ...
        swal("Error", "Error code - 316 <br> ", "error");
        document.onkeydown = function (e) { return true; }
        this.isDisableDiv = false;
      });
    }
