...
@Component({
  ...
  templateUrl: './quo-invp.component.html',
  styleUrls: ['./quo-invp.component.css']
})
export class QuoInvpComponent implements OnInit {

  @ViewChild(PersonalinfoComponent) invpPersonolInfoComponent: PersonalinfoComponent;
  @ViewChild(AditionalBenifInvpComponent) invpBenefictInfoComponent: AditionalBenifInvpComponent;
  ...
  sendQuo() {
    ...
    if (this._quotationCalculation._personalInfo != null) {
      ...
      this.saveInvpQuotationService.getQouCal(this._quotationCalculation).subscribe(response => {
          ...
          if (this.personalInfo._mainlife._mOccupation != "285") {
            if (this.activeSp == "1") {
              if (this.personalInfo._spouse._sOccupation != "285") {
                ...
                this.isDisableDiv = true;
                document.onkeydown = function (e) { return false; }
                this.saveInvpQuotationService.saveInvp(this._invpSaveQuotation).subscribe(response => {
                  this.isDisableDiv = false;
                  document.onkeydown = function (e) { return true; }
          ...
          if (this.personalInfo._mainlife._mOccupation != "285") {
            if (this.activeSp == "1") {
              if (this.personalInfo._spouse._sOccupation != "285") {
                ...
                this.isDisableDiv = true;
                document.onkeydown = function (e) { return false; }
                this.saveInvpQuotationService.editInvp(this._invpSaveQuotation, this.qdId).subscribe(response => {
                  this.isDisableDiv = false;
                  document.onkeydown = function (e) { return true; }
                  if (response.json().status == "Success") {
                    swal("Success", "Quotation has been saved Successfully <br> Quotation No : " + response.json().code, "success");
  ...
  clearquo() {
    this.invpBenefictInfoComponent.loadDefault();
    this.invpPersonolInfoComponent.loadDefault();
    this.sendQuo();
  }
  ...
  clearbenef(e) {
    ...
    this.invpBenefictInfoComponent.loadDefaultNew();
    //this.sendQuo();
  }

  clearbenefAll(e) {

    this.invpBenefictInfoComponent.loadDefaultBSAChange();
    //this.sendQuo();
  }
  ...
  calPreviousRiskM(e) {
    if (e.length > 0) {
      ...
      this.dashboardService.getSumAtRiskMainLife(e).subscribe(resp => {
        ...
        this.invpPersonolInfoComponent.loadDOBFromNic();
        document.onkeydown = function (e) { return true; }
        if (resp.json()) {
  ...
  calPreviousRiskS(e) {
    if (e.length > 0) {
      ...
      this.dashboardService.getSumAtRiskMainLife(e).subscribe(resp => {
        this.isDisableDiv = false;
        this.invpPersonolInfoComponent.loadSpouseDOBFromNic();
        document.onkeydown = function (e) { return true; }
        if (resp.json()) {
