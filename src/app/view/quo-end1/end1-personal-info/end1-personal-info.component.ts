...
@Component({
  selector: 'app-end1-personal-info',
  templateUrl: './end1-personal-info.component.html',
  styleUrls: ['./end1-personal-info.component.css']
})
export class End1PersonalInfoComponent implements OnInit {
  ...
  ngOnInit() {
    this.occupationService.loadOccupation().subscribe(response => {
      ...
      swal("Error", "Error code - 501 <br>","error");
    });
  ...
  change(e) {
    if(this._personalInfo._plan._term < 5 || this._personalInfo._plan._term > 45){
      swal("Check Form Again", "Term must be greater than 5 and less than 45", "error")
        .then((value) => {
          document.getElementById("txt_term").classList.add("errors");
          document.getElementById("txt_term").focus();
  ...
  loadDOBFromNic() {
    ...
    if (this._personalInfo._mainlife._mNic != null && this._personalInfo._mainlife._mNic.length > 0) {
      ...
      },error => {swal("Error", "Error code - 505 <br>","error")});
  ...
  loadSpouseDOBFromNic() {
    if (this._personalInfo._spouse._sNic != null && this._personalInfo._spouse._sNic.length > 0) {
      ...
      },error => {swal("Error", "Error code - 506 <br>","error")});
      this.checkValidity();
    } else {
      this.spouseForm.get("sDob").enable();
    }
  ...
  calculateAgeS(e) {
    if (this.spouseForm.get("sDob").valid) {
      this.ageCalculationService.loadAge(this._personalInfo._spouse._sDob).subscribe(response => {
        ...
        swal("Error", "Error code - 503 <br>","error");
      });
    } else {
      swal("Invalid Date Format!", "Example (30-01-1990)", "error");
      this.spouseForm.get("sDob").setValue("");
  ...
  calculateAgeM(e) {
    if (this.mainLifeForm.get("mDob").valid) {
      this.ageCalculationService.loadAge(this._personalInfo._mainlife._mDob).subscribe(response => {
        ...
        swal("Error", "Error code - 502 <br>","error");
      });
    } else {
      swal("Invalid Date Format!", "Example (30-01-1990)", "error");
      this.mainLifeForm.get("mDob").setValue("");
  ...
  calculateAgeC() {
    ...
    if(regex.test(this._children._cDob)){
      ...
      },error => {swal("Error", "Error code - 504 <br>","error")});
    }else{
      swal("Invalid Date Format!", "Example (30-01-1990)", "error");
      this._children._cDob = "";
      this.checkValidity();
