...
@Component({
  selector: 'app-quo-atp',
  templateUrl: './quo-atp.component.html',
  styleUrls: ['./quo-atp.component.css']
})
export class QuoAtpComponent implements OnInit {
  ...
  @ViewChild(AtpPersonalInfoComponent) end1PersonalInfoComponent: AtpPersonalInfoComponent;
  @ViewChild(AtpAdditionalBenefComponent) end1AdditionalBenefComponent: AtpAdditionalBenefComponent;


  constructor(private atpQuotationService: QuoAtpService, private loginService: LoginService, private router: Router, private route: ActivatedRoute, private dashboardService: DashboardService
  ...
  sendQuo() {
    if (this._quotationCalculation._personalInfo != null) {
      ...
      if (this.personalInfo._plan._term > 10 || this.personalInfo._plan._term < 5) {
        swal("Oopz...", "Term must be less than or equal 10 and greater than or equal 5 ", "error");
        return;
      }
  ...
  save(e: any) {
    ...
    } else if (this._plan._term < 5 || this._plan._term > 45) {
      swal("Check Form Again", "Term must be greater than 5 and less than 45", "error")
        .then((value) => {
        ...
        if (this.personalInfo._mainlife._mOccupation != "285") {
          if (this.activeSp == "1") {
            if (this.personalInfo._spouse._sOccupation != "285") {
              ...
              this.isDisableDiv = true;
              document.onkeydown = function (e) { return false; }
              this.atpQuotationService.saveAtp(this._invpSaveQuotation).subscribe(response => {
                this.isDisableDiv = false;
                document.onkeydown = function (e) { return true; }
  ...
  edit(e: any) {
    ...
    if (this.validity == true) {
      ...
      if (this._plan._term < 5 || this._plan._term > 45) {
        swal("Check Form Again", "Term must be greater than 5 and less than 45", "error")
          .then((value) => {
            document.getElementById("txt-term").classList.add("errors");
            document.getElementById("txt-term").focus();
      ...
      if (this.personalInfo._mainlife._mOccupation != "285") {
        ...
        if (this.activeSp == "1") {
          if (this.personalInfo._spouse._sOccupation != "285") {
            ...
            document.onkeydown = function (e) { return false; }
            //alert("e");
            this.atpQuotationService.editAtp(this._invpSaveQuotation, this.qdId).subscribe(response => {
              ...
              if (response.json().status == "Success") {
                ...
                swal("Oopz...", response.json().status, "error");
                console.log(this._invpSaveQuotation._personalInfo);
              }
            }, error => {
              swal("Error", error.text(), "error");
              document.onkeydown = function (e) { return true; }
  ...
  clearquo() {
    ...
    this.end1AdditionalBenefComponent.loadDefault();
    this.end1PersonalInfoComponent.loadDefault();
    this.sendQuo();
  }

  clearbenef(e) {

    this.end1AdditionalBenefComponent.loadDefaultNew();
    //this.sendQuo();
  }

  clearbenefAll(e) {

    this.end1AdditionalBenefComponent.loadDefaultBSAChange();
    //this.sendQuo();
  }
