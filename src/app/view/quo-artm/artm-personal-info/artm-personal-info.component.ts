...
@Component({
  selector: 'app-artm-personal-info',
  templateUrl: './artm-personal-info.component.html',
  styleUrls: ['./artm-personal-info.component.css']
})
export class ArtmPersonalInfoComponent implements OnInit {
  ...
  @Output() mPreviousSumAtRisk = new EventEmitter<any>();
  @Output() sPreviousSumAtRisk = new EventEmitter<any>();


  constructor(private occupationService: OccupationService,
    private ageCalculationService: AgeCalculationService, private dashboardService: DashboardService) {
  ...
  ngOnInit() {
    ...
    },error => {swal("Error", "Error code - 1601 <br>","error")});
  ...
  setContribution(){
    if (this._personalInfo._plan._frequance == "Single Premium") {
      ...
      if(this._personalInfo._plan._frequance == "Monthly"){
        if(this._personalInfo._plan._bsa < 3000){
          this._personalInfo._plan._bsa=3000;
        }
      }else if(this._personalInfo._plan._frequance == "Quartaly"){
        if(this._personalInfo._plan._bsa < 9000){
          this._personalInfo._plan._bsa=9000;
        }
      }else if(this._personalInfo._plan._frequance == "Half Yearly"){
        if(this._personalInfo._plan._bsa < 18000){
          this._personalInfo._plan._bsa=18000;
        }
      }else if(this._personalInfo._plan._frequance == "Yearly"){
        if(this._personalInfo._plan._bsa < 36000){
          this._personalInfo._plan._bsa=36000;
        }
      }
    }
  }
        ...
        if ((this._personalInfo._spouse._sAge != null && this._personalInfo._spouse._sAge != undefined
          ...
          if (this._personalInfo._childrenList.length > 0) {
            this.calObj = {
              ...
              age: this._personalInfo._mainlife._mAge,
              pensionPaingTerm: this._personalInfo._plan.pensionPaingTerm,
              retAge: this._personalInfo._plan.retAge,
              payingterm: this._personalInfo._plan._payingterm
            }
  ...
  loadDOBFromNic() {
    ...
    if (this._personalInfo._mainlife._mNic != null && this._personalInfo._mainlife._mNic.length > 0) {
      ...
      },error => {swal("Error", "Error code - 1605 <br>","error")});
  ...
  loadSpouseDOBFromNic() {
    if (this._personalInfo._spouse._sNic != null && this._personalInfo._spouse._sNic.length > 0) {
      ...
      },error => {swal("Error", "Error code - 1606 <br>","error")});
      this.checkValidity();
    } else {
      this.spouseForm.get("sDob").enable();
    }
  ...
  calculateAgeS(e) {
    if (this.spouseForm.get("sDob").valid) {
      this.ageCalculationService.loadAge(this._personalInfo._spouse._sDob).subscribe(response => {
        ...
        this.check();
      },error => {swal("Error", "Error code - 1603 <br>","error")});
    } else {
      swal("Invalid Date Format!", "Example (30-01-1990)", "error");
      this.spouseForm.get("sDob").setValue("");
      this.checkValidity();
  ...
  calculateAgeM(e) {
    if (this.mainLifeForm.get("mDob").valid) {
      this.ageCalculationService.loadAge(this._personalInfo._mainlife._mDob).subscribe(response => {
        ...
        this.check();
      },error => {swal("Error", "Error code - 1602 <br>","error")});
    } else {
      swal("Invalid Date Format!", "Example (30-01-1990)", "error");
      this.mainLifeForm.get("mDob").setValue("");
      this.checkValidity();
  ...
  calculateAgeC() {
    ...
    if(regex.test(this._children._cDob)){
      ...
      },error => {swal("Error", "Error code - 1604 <br>","error")});
    }else{
      swal("Invalid Date Format!", "Example (30-01-1990)", "error");
      this._children._cDob = "";
      this.checkValidity();
  ...
  checkTitle() {
    ...
    if (gender == "M" && (title == "MR" || title == "DR")) {
      ...
      if (this.mainLifeForm.get("mGender").value == "M") {
  ...
  loadDefault() {
    ...
    this._personalInfo._plan._term = 22;
    this._personalInfo._plan.retAge = 40;
    this._personalInfo._plan._payingterm = "10";
    this._personalInfo._plan.pensionPaingTerm = 10;
