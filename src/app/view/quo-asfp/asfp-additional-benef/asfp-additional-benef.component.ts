...
import { CalPersonalInfoArp, CalPersonalInfo } from '../../../model/quoCal';
import { BenefitsValidations } from '../../../validation/benefitsValidations';

@Component({
  selector: 'app-asfp-additional-benef',
  templateUrl: './asfp-additional-benef.component.html',
  styleUrls: ['./asfp-additional-benef.component.css']
})
export class AsfpAdditionalBenefComponent implements OnInit {
  ...
  @Input() isImgSFPOGActive = true;
  @Input() isImgSFPOActive = false;
  ...
  imgHrbGUrl = "assets/images/hrbG.png";
  imgHrbUrl = "assets/images/hrb.png";
  ...
  imgSfpoGUrl = "assets/images/sfpoG.png";
  imgSfpoUrl = "assets/images/sfpo.png";
  ...
  isActiveSFPO(e) {
    if (e.target.checked) {
      ...
      this.isImgSFPOGActive = true;
      this.isImgSFPOActive = false;
      this.clearSFPO();
    }
  }
  ...
  clearSFPO() {
    this.abmForm.get('SFPO').get('sumAssured').setValue('0');
    this.abmForm.get('SFPO').get('premium').setValue('0');
  ...
  checkValidateADB() {
    this.validateADB = this.benefitsValidations.validateadbAsfp(this.personalInfo.bsa, this.abmForm.get('ADB').get('sumAssured').value);
    ...
    if (this.validateADB == 1) {
      ...
      swal("Error!", "ADB must be less than or equal (500000 x 6) and ADB mod 25000 equal 0 and ADB must be greater than or equal 500,000", "error");
  ...
  checkValidateSFPO() {
    this.validateSFPO = this.benefitsValidations.validatesfpo(this.personalInfo.bsa, this.abmForm.get('SFPO').get('sumAssured').value);

    if (this.validateSFPO == 1) {

    } else {
      swal("Error!", "SFPO must be greater than or equal 250,000 and SFPO must be multi value of 25,000", "error");
  ...
  checkValidateATPB() {
    this.validateATPB = this.benefitsValidations.validatteAtpbASFP(this.personalInfo.bsa, this.abmForm.get('ATPB').get('sumAssured').value);
    if (this.validateATPB == 1) {
      if (this.abmForm.get('CIB').get('isActice').value) {
        this.checkValidateCIB();
      }
    } else {
      swal("Error!", "ATPB must be greater than or equal 500,000 and ATPB must be less than or equal (500,000 x 10) and ATPB mod 25,000 equal 0", "error");
