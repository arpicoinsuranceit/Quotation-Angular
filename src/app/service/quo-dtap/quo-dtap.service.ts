...
export class QuoDtapService {
  ...
  getQouCal(data: QuotationCalculation) {
    ...
    data._product = "DTA";
    //console.log(data);
    return this.http.post('http://localhost:8084/quoDtapCal', data);
  }

  saveDta(data: any) {
    data._product = "DTA";
    //console.log(data);
    return this.http.post('http://localhost:8084/quoDtapsave/' + this.userId, data);
  }
  ...
  editDta(data: any, qdId: number) {
    ...
    if(sessionStorage.getItem("isUnderwriting") == "true"){
      ...
      return this.http.post('http://localhost:8084/quoDtapEditUnderwrite/'+token+'/'+qdId, data);
    }

    return this.http.post('http://localhost:8084/quoDtapEdit/' + this.userId + '/' + qdId, data);
  }
}
