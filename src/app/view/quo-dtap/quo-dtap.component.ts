...
@Component({
  selector: 'app-quo-dtap',
  templateUrl: './quo-dtap.component.html',
  styleUrls: ['./quo-dtap.component.css']
})
export class QuoDtapComponent implements OnInit {
  ...
  @ViewChild(DtapPersonalInfoComponent) dtaPersonalInfoComponent: DtapPersonalInfoComponent;
  @ViewChild(DtapBenefictInfoComponent) dtaBenefictInfoComponent: DtapBenefictInfoComponent;

  constructor(private loginService: LoginService, private saveDtaQuotationService: QuoDtapService, private router: Router, private route: ActivatedRoute,
    ...
    this.personalInfo._spouse._sTitle="MRS";
...
//console.log(this.personalInfo._mainlife._mAge);
  ...
  setBsa(e) {
    ...
    this.dtaBenefictInfoComponent.setBsa(e);
  }
  ...
  onsetBenifAll(allForm) {
    ...
    this.riderDetails._mRiders = new Array();
    for (var i in this.benif.value) {
      let benifict = this.benif.value[i];
      if (benifict.isActice == true) {
        if (benifict.sumAssured) {
          let benifict1 = new Benifict();
          benifict1.active = "true";
